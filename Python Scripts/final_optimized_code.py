...
def __init__(self):
    ...
    # ... rest of the initialization code ...
        ...
        class App(customtkinter.CTk):def __init__(self):
super().__init__()
self.width = 900
self.height = 600
...
self.textbox.configure(state="disabled")self.image_datas_Log,self.image_ids_Log,self.timestamps_Log,self.plate_formats_Log,self.image_ids_Accepted,self.timestamps_Accepted,self.plate_formats_Accepted = DatabaseManager.retrieve_images_from_database()
...
self.cursor.execute(query, (plate_format,))
...
VALUES (%s, %s, %s, %s);
"""
self.cursor.execute(query, ( psycopg2.Binary(image_data), label, timestamp, plate_format))
conn.commit()
...
self.cursor.execute(delete_query, (plate_format,))
conn.commit()
...
#                                 text=str(self.timestamps_Log[i])+ " | " + str(self.plate_formats_Log[i])+ " | " + "NO ACCESS",border_width=1,
#                                 command=lambda index=i: change_image(index))
def load_Accepted_current(self):
plate_format = self.plate_formats_Log[self.current_image_index]
...
text=plate_text,
border_width=1,
command=lambda: change_delete_index(len(self.plate_formats_Accepted)+1)
))
self.button_accepted[-1].pack()
def on_button_click(event):
button = event.widget
index = self.plate_formats_Access.index(button)
print("Button clicked at index:", index)
